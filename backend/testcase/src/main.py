...
from .llm import build_llm, generate_outline, generate_cases_batch
...
def main() -> int:
    ...
    llm = build_llm(cfg)
    logger.info("已使用 DeepSeek 模型：%s", cfg.deepseek_model)
    ...
    for i, path in enumerate(files, 1):
        try:
            ...
            console.print(f"[bold]生成[/bold]大纲（DeepSeek：{cfg.deepseek_model}）")
